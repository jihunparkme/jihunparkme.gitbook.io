...
**템플릿 콜백 패턴**: 코드 중에서 변경이 거의 일어나지 않으며 일정한 패턴으로 유지되는 특성을 가진 부분(`템플릿`)을 자유롭게 변경되는 성질을 가진 부분(`콜백`)으로부터 독립시켜서 효과적으로 활용할 수 있도록 하는 방법

**템플릿 메소드 패턴**: 상속을 이용해서 고정된 템플릿과 변경 가능한 훅 메소드 등으로 분리하는 패턴

- 템플릿과 콜백을 사용하는 방식은 프레임워크에 적용되는 제어의 역전으로 설명 가능
- 컨텍스트에 전략을 교체해서 쓰는 전략 패턴의 특별한 케이스로 볼 수도 있음
