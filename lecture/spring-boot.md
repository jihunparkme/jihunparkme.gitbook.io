...
영한님의 [스프링 부트 - 핵심 원리와 활용](https://www.inflearn.com/course/%EC%8A%A4%ED%94%84%EB%A7%81%EB%B6%80%ED%8A%B8-%ED%95%B5%EC%8B%AC%EC%9B%90%EB%A6%AC-%ED%99%9C%EC%9A%A9) 강의를 요약한 내용입니다.
...
- [핀포인트](https://github.com/pinpoint-apm/pinpoint)(오픈소스) - 추천
- 스카우트(오픈소스)
- 와탭(상용)
- 제니퍼(상용)
...
- 일반 로그와 에러 로그는 파일을 구분해서 -> 에러 로그만 확인해서 문제를 바로 정리 가능
...
- 모니터링은 각각 용도가 다르다. 관찰을 할 때는 전체에서 점점 좁게
- 핀포인트 강추. 마이크로 서비스 분산 모니터링, 대용량 트래픽 대응 가능

**`알람`**
- 모니터링 툴에서 일정 이상 수치가 넘어가면, 슬랙, 문자 등 연동
  
알람은 2가지 종류(경고, 심각)로 꼭 구분해서 관리
- 경고는 하루 1번 정도 사람이 직접 확인해도 되는 수준 (사람이 들어가서 확인)
- 심각은 즉시 확인 필요. 슬랙 알림, 문자, 전화
...
경고와 심각을 잘 나누어서 업무와 삶에 방해가 되지 않도록 하자.