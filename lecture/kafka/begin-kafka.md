...
<figure><img src="../../.gitbook/assets/kafka/end-to-end-architecture.png" alt=""><figcaption></figcaption></figure>
...
<figure><img src="../../.gitbook/assets/kafka/lambda-architecture.png" alt=""><figcaption></figcaption></figure>

**⚠️ 한계점**

- 데이터를 배치 처리하는 레이어와 실시간 처리하는 레이어를 분리한 방식은 데이터 처리 방식을 명확히 나눌 수 있었지만 ***레이어가 결국 두 개로 나뉘기 때문에*** 생기는 단점이 존재
    - 데이터 분석, 처리에 필요한 로직이 두 벌로 ***각각의 레이어에 따로 존재***해야 함
    - 배치 데이터와 실시간 데이터를 융합하여 처리할 때는 ***다소 유연하지 못한 파이프라인을 생성***해야 함
...
<figure><img src="../../.gitbook/assets/kafka/kappa-architecture.png" alt=""><figcaption></figcaption></figure>
...
<figure><img src="../../.gitbook/assets/kafka/streaming-data-lake.png" alt=""><figcaption></figcaption></figure>
...
데이터파이프라인 처리를 위해 redis, rabbitMQ 등 여러 애플리케이션들을 조합해서 사용해야 했다.

- 이런 애플리케이션 각각의 특징이 뚜렷하다보니 데이터 처리에 있어 ***파편화도 심각***
- 파편화는 결국 유지보수의 어려움으로 직결되었고, 유지보수의 어려움은 비즈니스 성장에 영향

> 이러한 걸림돌을 타파하기 위해 만들어진 것이 `카프카`

- 카프카는 데이터 처리를 각각 여러 애플리케이션에서 처리하는 것이 아니라 중앙의 한 곳에서 처리할 수 있도록 `중앙집중화`한 것
- 카프카를 통해 웹사이트, 애플리케이션, 센서 등에서 취합한 데이터 스트림을 `한 곳에서 실시간으로 관리`
- 카프카는 대용량 데이터를 수집하고, 사용자들이 실시간 스트림으로 소비할 수 있게 만들어주는 일종의 `중추 신경`으로 작동

카프카가 많이 사용되고 있는 특징

**1️⃣ High throughput message capacity**

- 짧은 시간 내에 엄청난 양의 데이터를 컨슈머까지 전달
- 파티션을 통한 분산처리가 가능하므로 데이터 양이 많아질수록 컨슈머 개수를 늘려 병렬 처리가 가능
- 병렬 처리를 통해 데이터를 더욱 빠르게 처리

**2️⃣ Scalability and Fault tolerant**

- 카프카는 뛰어난 확장성
- 이미 사용되고 있는 카프카 브로커가 있더라도 신규 브로커 서버를 추가해서 수평 확장이 가능
- 여러 브로커 중 몇 대가 죽더라도 이미 replica로 복제된 데이터는 안전하게 보관되어 복구하여 처리 가능

**3️⃣ Undeleted log**

- 카프카 토픽에 들어간 데이터는 컨슈머가 데이터를 가지고 가더라도 데이터가 사라지지 않음
    - 만일 다른 플랫폼에서 이러한 처리를 하려고 했다면 복잡한 방식이 필요
- 카프카에서는 컨슈머의 그룹 아이디만 다르다면 동일한 데이터도 각각 다른 형태로 처리

아파치 카프카의 생태계는 꾸준히 발전

- 기본 개념인 컨슈머, 프로듀서 뿐만 아니라
- 카프카 스트림, 커넥터, ksqldb, avro, rest proxy 등
- 카프카에 적합한 여러 요소가 지속적으로 개발되고 발전
- 또한, fluentd, telegraf 등 여러 서드파티들에서도 카프카와의 연동을 정식으로 지원