...
때로는 파일을 수정하지 않고 읽기만 할 수 있는 스킬이 필요할 때가 있습니다. 이는 보안에 민감한 워크플로, 읽기 전용 작업 또는 안전장치가 필요한 모든 상황에서 유용합니다.
...
이 예시에서 해당 `allowed-tools`필드는 `Read, Grep, Glob, Bash`로 설정되어 있습니다. 
이 스킬이 활성화되면 클로드는 허락을 구할 필요 없이 해당 도구만 사용할 수 있습니다. 편집이나 글쓰기는 불가능합니다.

해당 설정을 완전히 생략하면 allowed-tools스킬 사용에 아무런 제약이 없습니다. 클로드는 일반적인 권한 모델을 사용합니다.