...
| **섹션 제목**                 | **요약 내용**                                                                                                                                                      |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Overview of Claude models** | Claude의 세 가지 모델(Opus, Sonnet, Haiku)의 특징과 차이점을 설명. Opus는 높은 지능, Sonnet은 균형, Haiku는 속도와 비용 효율성에 중점.                             |
| **Accessing the API**         | Anthropic API를 통해 Claude와 상호작용하는 5단계 요청 흐름을 설명. 서버를 통해 API 키를 보호하며, SDK를 사용해 요청을 처리.                                        |
| **Making a request**          | API 키 설정, 환경 변수 로드, 요청 생성 방법을 설명. `client.messages.create()` 함수로 모델, 토큰 제한, 메시지를 포함한 요청을 생성.                                |
| **Multi-Turn conversations**  | Claude는 상태 비저장(stateless) 방식으로 작동하므로 대화 맥락을 유지하려면 모든 메시지 기록을 포함해야 함. 이를 관리하기 위한 도우미 함수와 대화 흐름 예제를 제공. |
| **System prompts**            | Claude의 응답 스타일을 사용자 지정하는 방법. 시스템 프롬프트를 사용해 특정 역할(예: 수학 튜터)을 부여하고, 이를 코드로 구현하는 방법을 설명.                       |
| **Temperature**               | `Temperature` 매개변수를 통해 응답의 창의성과 예측 가능성을 조정. 낮은 값은 결정론적이고 높은 값은 창의적. 다양한 작업에 적합한 온도 범위를 제안.                  |
| **Response streaming**        | 응답 스트리밍을 통해 Claude의 생성 텍스트를 실시간으로 표시하여 사용자 경험을 개선. 스트리밍 이벤트의 구조와 구현 방법을 설명.                                     |
| **Controlling model output**  | 미리 채워진 어시스턴트 메시지(prefilling)와 중지 시퀀스(stop sequences)를 사용해 Claude의 출력을 제어. 특정 형식이나 길이의 응답을 생성하도록 조정 가능.           |
| **Structured data**           | Claude가 JSON, 코드, 목록 등 구조화된 데이터를 생성할 때 불필요한 설명 텍스트를 제거하는 방법. Prefilling과 Stop Sequences를 결합해                                |