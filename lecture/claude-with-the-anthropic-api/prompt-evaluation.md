...
`Code grading`는 AI 생성 응답의 두 가지 주요 측면을 검증합니다.

**Format**
- 응답은 설명 없이 요청된 코드 유형(파이썬, JSON 또는 Regex)만 반환해야 합니다

**Valid Syntax**
- 생성된 코드는 실제로 의도한 언어로 올바르게 구문 분석해야 합니다
...
code grading을 구현한 후에는 평가를 실행하여 기본 점수를 얻습니다. 
- 점수 자체가 본질적으로 좋거나 나쁜 것은 아니며, 중요한 것은 프롬프트를 세분화하여 점수를 개선할 수 있는지 여부입니다. 
