...
코드 등급 매기기는 AI 생성 응답의 두 가지 주요 측면을 검증합니다: 형식 - 응답은 설명 없이 요청된 코드 유형(파이썬, JSON 또는 Regex)만 반환해야 합니다. 유효한 구문 - 생성된 코드는 실제로 의도한 언어로 올바르게 구문 분석해야 합니다. 응답은 요청된 내용을 직접 다루고 정확해야 합니다. 처음 두 가지 기준은 코드 그레이더가 처리하고, 작업 팔로우는 모델 그레이더가 평가합니다. 이들은 종합적인 평가를 제공합니다.