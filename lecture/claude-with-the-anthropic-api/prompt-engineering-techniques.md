...
공식 XML 태그를 사용할 필요가 없습니다. 콘텐츠에 맞는 설명적인 이름을 만드세요:

- `<sales_records>`가 `<data>`보다 낫습니다
- `<athlete_information>`은 사용자 세부 정보를 명확하게 식별합니다
- `<my_code>`와 `<docs>`는 서로 다른 유형의 콘텐츠를 분리합니다

**태그 이름이 더 구체적이고 설명적일수록 클로드는 각 섹션의 목적을 더 잘 이해**할 수 있습니다.
...
- 대량의 컨텍스트 또는 데이터 포함
- 다양한 유형의 콘텐츠(코드, 문서, 데이터) 혼합
- 콘텐츠 경계에 대해 더 명확하게 설명하고 싶을 경우
