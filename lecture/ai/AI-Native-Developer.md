...
- **`PreToolUse`**
  - 코드에 API 키나 비밀번호 같은 민감 정보를 포함시키려 할 때, AI가 커밋은 물론 코드 작성 단계에서부터 이를 감지하고 즉시 차단
  - 파일 수정/생성 등 특정 도구를 사용하기 직전에 실행
...
- **`PostToolUse`**
  - AI가 생성한 코드 작업이 끝나는 즉시 자동으로 코드를 포맷팅
  - Claude가 파일 수정 같은 작업을 성공적으로 마친 직후에 실행
  
    <details>
    <summary>.claude/hooks/format-files.sh</summary>
    ...
    "hooks": {
        "PostToolUse": [
        {
            ...
            {
                "type": "command",
                "command": "bash .claude/hooks/format-files.sh"
            }
            ]
        }
        ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node .claude/hooks/dispatcher.js"
          }
        ]
      }
    ]
...
<details>
<summary>'디스패처' 스크립트의 두뇌 구현하기</summary>

```js
const fs = require('fs');
const path = require('path'); // 경로 처리를 위한 path 모듈 임포트

// 훅 실행 로깅 (항상 출력)
console.error('🚀 디스패처 실행됨!');

let input = ''; // catch 블록에서도 접근할 수 있도록 바깥에 선언
...
> 딱 필요한 순간에만 매뉴얼을 꺼내 읽기 때문에 토큰 비용은 극적으로 아끼고, 결과물의 품질은 소름 돋게 좋아짐

레벨 1 (메타데이터 로드 — 도서 목록 검색)
- 처음 켜질 때 설치된 수십 개의 스킬 중 이름(Name)과 짧은 설명(Description)만 가볍게 훑음

레벨 2 (본문 로드 — 목차와 서문 읽기)
- 관련 있는 스킬을 판단하고, 그 매뉴얼의 본문 전체를 정독

레벨 3 (추가 파일/스크립트 로드 — 부록 펼치기)
- 본문에 특정 파일을 참고하라고 하면, 그때서야 필요한 양식 파일만 쏙 빼서 작업을 진
  
## 결정론적(Deterministic) 제어 

> Skills는 단순히 프롬프트를 저장하는 용도를 넘어, AI의 창의성(글쓰기)과 프로그래밍의 정확성(코드 실행)을 완벽하게 결합해 주는 궁극의 통제 수단

스킬 폴더 안에 '파이썬 코드'를 숨겨두는 방식
