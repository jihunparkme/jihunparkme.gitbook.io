...
* 기성 API 게이트웨이 제품/서비스 활용: 개발 노력은 들지 않지만 유연성은 제일 떨어집니다. 가령 기성 API 게이트웨이 제품은 대체로 API 조합을 지원하지 않습니다.
* API 게이트웨이 프레임워크 또는 웹 프레임워크 기반으로 API 게이트웨이를 직접 개발: 가장 유연한 접근 방식이지만, 적잖은 개발 노력이 투입되어야 합니다.
