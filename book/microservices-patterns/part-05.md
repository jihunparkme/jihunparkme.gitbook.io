...
`OrderService` 클래스는 Order Service의 **주요 진입점**으로, Order 애그리거트의 생성 및 업데이트를 위한 메서드를 정의합니다.  
이 서비스는 대부분의 메서드가 **사가를 생성하여 Order 애그리거트의 오케스트레이션을 담당**하기 때문에 KitchenService보다 복잡합니다.

* **의존성**: `OrderRepository`, `OrderDomainEventPublisher`, 여러 사가 매니저가 주입됩니다.
* **`createOrder()` 메서드**:
  * 먼저 레스토랑 정보를 찾습니다.
  * `Order.createOrder()`를 호출하여 Order 애그리거트를 생성하고 영속화합니다.
  * 애그리거트가 발행한 도메인 이벤트를 발행합니다.
  * **`CreateOrderSaga`를 생성하여 시작**합니다.
* **`reviseOrder()` 메서드**:
  * 대상 Order를 검색합니다.
  * **`ReviseOrderSaga`를 생성하여 시작**합니다.

전반적으로, 마이크로서비스 아키텍처에서 비즈니스 로직은 서비스, JPA 엔티티, 레포지토리 등 전통적인 방식과 유사한 클래스로 구성되지만, 몇 가지 중요한 차이점이 있습니다:

*   **DDD 애그리거트**를 사용하여 도메인 모델을 구성하며, 이는 특정 설계 제약 사항을 부과합니다.
*   전통적인 객체 모델과 달리, 다른 애그리거트 내의 클래스 간 **참조는 객체 참조 대신 기본 키 값으로** 이루어집니다.
*   **하나의 ACID 트랜잭션은 단일 애그리거트만 생성하거나 업데이트**할 수 있습니다.
*   애그리거트가 상태를 변경할 때 **도메인 이벤트를 발행**하는 것이 유용합니다.
*   서비스는 **사가를 자주 사용하여 여러 서비스 간의 데이터 일관성을 유지**합니다. Order Service는 특히 주문 생성 및 업데이트 시 사가에 크게 의존합니다.

## 마치며

- 비즈니스 로직이 단순하다면 절차적 트랜잭션 스크립트 패턴도 괜찮은 방법이지만, 복잡한 비즈니스 로직은 객체 지향적 도메인 모델 패턴에 따라 구현하는 것이 좋습니다.
- 서비스의 비즈니스 로직은 DDD 애그리거트들로 구성하는 것이 좋습니다.
  - DDD 애그리거트는 도메인 모델을 모듈화하고, 서비스 간 객체 참조 가능성을 배제하며, 전체 ACID 트랜잭션을 서비스 내부에 국한시키므로 유용합니다.
- 애그리거트는 도메인 이벤트 생성/수정 시 이벤트를 발행합니다.
  - 도메인 이벤트는 활용 범위가 다양합니다. 4장에서는 코레오그래피 사가 도메인 이벤트로 구현했고, 7장에서는 데이터 레플리카를 도메인 이벤트로 업데이트하는 방법을 설명할 예정입니다.
