...
AttributeConverter는 다음과 같이 밸류 타입과 칼럼 데이터 간의 변환을 처리하기 위한 기능을 정의
- X는 밸류 타입
- Y는 DB 타입
...
public interface AttributeConverter<X, Y> {
    Y convertToDatabaseColumn(X var1); // 밸류 타입을 DB 칼럼 값으로 변환
    X convertToEntityAttribute(Y var1); // DB 칼럼 값을 밸류로 변환
}
```
...
- `@OrderColumn`을 이용해서 지정한 칼럼에 리스트의 인덱스 값을 저장.(List 타입 자체가 인덱스를 가지고 있다)
