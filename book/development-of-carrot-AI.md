...
## 모든 당근 사용자에게 AI 에이전트 제공하기

||내용|
|---|---|
|서비스 소개|고객 문의 사항에 대해 적절한 가이드를 제공하여 사용자가 스스로 해결할 수 있도록 돕는 기능|
|목표|**맞춤형 AI 에이전트**를 적용하여 사용자에게 최적화된 고객 서비스를 제공<br/>- 시스템에 들어오는 모든 문의를 AI 에이전트가 들여다본 후, **각각의 언어와 문화에 맞게 24시간 이내에 답변**해 줄 수 있는 환경을 구축|
|적용 기술|**AI Agent**|
|효과||
...
1.  **모델 (Model):** 시고와 계획을 실행할 수 있는 높은 수준의 **대규모 언어 모델(LLM)**을 활용합니다.
2.  **도구 (Tool):** 에이전트가 특정 작업을 수행하기 위해 호출할 수 있는 **함수 혹은 API**를 의미합니다. 이 도구들은 REST API, gRPC, GraphQL 등으로 지정될 수 있으며, 내부 데이터 연동을 위해 **gRPC 표준 엔드포인트**를 활용하여 구현했습니다.
3.  **지시사항 (Instructions, Prompt):** 에이전트의 목표를 설정하고 동작을 제어하는 가이드 역할을 합니다.

특히, 여러 AI 에이전트가 협업하여 복잡한 작업을 효율적으로 처리하는 **멀티 AI 에이전트 시스템**이 적용되었으며, 이를 구현하기 위해 **KAMP (Karrot Agent Management Platform)**라는 플랫폼을 구축했습니다. KAMP는 AI 에이전트를 **노코드/로코드(No-code/Low-code)** 방식으로 구성할 수 있도록 지원합니다.

### **AI 를 적용하고 어떤 효과를 얻을 수 있었는지 (What effects were achieved by applying AI)**

AI 에이전트 시스템을 적용함으로써 다음과 같은 효과를 얻을 수 있습니다 (혹은 기대됩니다):

1.  **자체 해결률 향상 및 부하 감소:** 기존에도 제공되는 가이드를 통해 **고객 문의의 약 40%**는 사용자가 스스로 해결할 수 있었으나, AI 에이전트가 이를 더욱 강화할 수 있습니다.
2.  **유연한 시스템 확장 및 안정성:** **멀티 AI 에이전트 시스템**은 에이전트들이 독립적으로 움직이도록 설계되어, 새로운 정책이 생기거나 기존 정책이 변경되더라도 **관련 에이전트만 새로 학습**시키면 되므로 전체 시스템에 미치는 영향이 최소화됩니다.
3.  **예측 가능성 향상:** 에이전트 간의 **명확한 소통 체계**를 정의함으로써, 불필요한 커뮤니케이션을 최소화하고 **예측 가능성을 높여** 전체 시스템의 신뢰성을 향상시킬 수 있습니다.
4.  **신뢰성 있는 운영 환경 구축:** 빠르게 성장하고 예상치 못한 상황이 발생할 수 있는 C2C 도메인에서 멀티 AI 에이전트 시스템이 **CS 전문가 역할**을 수행할 수 있는 강점을 활용하여, 전체 시스템에 영향을 최소화하면서 일관된 운영이 가능합니다.
