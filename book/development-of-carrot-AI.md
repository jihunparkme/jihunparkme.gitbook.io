...
||내용|
|---|---|
|서비스 소개|채팅창에서 문장을 자동으로 추천해 주는 AI 메시지 추천 기능에 시맨틱 캐싱 도입|
|목표|AI 기능을 유지하면서 프롬프트 호출 비용만 **연간 8~9억 원**에 달하는 **과도한 LLM 비용을 효율적으로 절감**|
|적용 기술|**`시맨틱 캐싱`(Semantic Caching)**<br/>- 단순한 문자열 일치가 아니라 **의미 기반의 유사도**를 활용<br/>- 코사인 유사도와 같은 거리 유사도 측정 방법을 사용하여 문장의 의미가 가까울 경우, 문장의 형태가 다르더라도 캐시 히트로 처리하여 **LLM 호출 없이 저장된 데이터를 반환**<br/><br/>**구현 기술**<br/>- **`임베딩 및 벡터화`:** 비정형 텍스트 데이터를 기계가 이해할 수 있는 벡터 공간으로 변환 (이를 위해 텍스트 임베딩 모델을 사용하며, 문장 하나당 수백~수천 차원의 벡터로 표현)<br/>- **`차원 축소`:** 계산 효율성을 높이기 위해 PCA(주성분 분석, Principal Component Analysis) 등의 머신러닝 기법을 적용하여 벡터의 차원을 축소<br/>- **`벡터 데이터베이스`:** 캐시 데이터를 저장하기 위해 벡터 데이터베이스를 선택 (외부 솔루션 대신 기존 시스템 환경 내부에 통합된 임베딩 방식인 [chromem-go](https://github.com/philippgille/chromem-go) 선택)<br/>- **`시스템 아키텍처`:** 기존 서비스 로직에 영향을 주지 않도록 **독립적인 서버** 로 분리되어 gRPC 프로토콜을 통해 통신하도록 설계<br/>- **`검색 최적화`:** 빠른 검색 속도를 위해 **Approximate Nearest Neighbor Search**와 같은 근사 최근접 이웃 검색 기법을 적용|
...
|목표|- **데이터 처리 자동화 및 효율화:** 매일 대량으로 들어오는 사용자 의견(예: 문의, 리뷰, 의견 남기기 등)을 수동으로 분석하고 분류하는 데 필요한 많은 리소스를 줄이기<br/>- **실질적인 인사이트 도출:** 실제로 행동할 수 있는 **실질적인 인사이트** 를 도출하고 공유할 수 있도록 지원|
...
||내용|
|---|---|
|서비스 소개|고객 문의 사항에 대해 적절한 가이드를 제공하여 사용자가 스스로 해결할 수 있도록 지원|
|목표|**AI 에이전트**를 적용하여 사용자에게 최적화된 고객 서비스 제공<br/>- 시스템에 들어오는 모든 문의를 AI 에이전트가 들여다본 후, **각각의 언어와 문화에 맞게 24시간 이내에 답변**해 줄 수 있는 환경을 구축|
|적용 기술|**AI Agent**<br/>- **모델(Model):** **`LLM`** 활용<br/>- **도구(Tool):** `REST API`, `gRPC`, `GraphQL`<br/>- **지시사항(Instructions, Prompt):** 에이전트의 목표를 설정하고 동작을 제어<br/><br/>**멀티 AI 에이전트 시스템** 적용<br/>- 구현을 위해 **KAMP(Karrot Agent Management Platform)** 플랫폼 구축<br/>- AI 에이전트를 **No-code/Low-code** 방식으로 구성할 수 있도록 지원|
|효과|- **자체 해결률 향상 및 부하 감소:** AI 에이전트가 사용자가 스스로 문의를 해결<br/>- **신뢰성 있는 운영 환경 구축:** C2C 도메인에서 멀티 AI 에이전트 시스템이 **CS 전문가 역할**을 수행할 수 있는 강점을 활용하여, 전체 시스템에 영향을 최소화하면서 일관된 운영|
...
||내용|
|---|---|
|서비스 소개|**KAMP**를 활용하여 당근 서비스에 특화된 기술과 생성형 AI 및 최신 기술을 접목하여 **사용자의 복잡한 요청을 해결**|
|목표|- **고객 문의의 효율적 해결:** 고객 문의를 효율적으로 해결하고, 단순 반복 문의를 넘어선 복잡한 문의까지 처리하여 **사용자 경험을 크게 개선**<br/>- **개인화된 경험 제공:** 사용자 개개인에게 맞춤화된 답변과 서비스 경험을 제공<br/>- **정책 및 계정 정보의 명확한 설명:** 정책이 복잡해지고 운영 시스템이 복잡해짐에 따라, 사용자들이 이용 제한과 관련하여 합리적이지 않다고 느끼는 경우가 증가하여, AI 에이전트를 통해 **이용 제한에 대한 상세하고 깊이 있는 설명**을 제공|
|적용 기술|**멀티 AI 에이전트 시스템:**<br>- 사용자의 질문을 분석하고 의도에 맞게 정보를 호출하도록 **메인 에이전트**가 **계정 에이전트**와 **이용 제한 에이전트**와 같은 두 전문 에이전트를 연결하고 조정하는 방식으로 구성<br/><br/>**LLM 및 RAG(검색 증강 생성):**<br/>- LLM을 기반으로 답변을 생성하며, **최신 정보를 유지**하고 내부 정보를 검색하여 신뢰도를 확보하기 위해 **RAG 기술**을 사용<br/>- 답변 생성 시 실시간으로 외부 문서나 데이터베이스에서 관련 정보를 검색하여 참조
|효과|- **응답 시간 감소:** 여러 도구를 **동시에 호출**하는 병렬 방식을 적용하여 응답 시간을 단축<br/>- **고객 문의 감소:**<br/>- **상세하고 체계적인 설명 제공:** '저 왜 제재됐나요?'와 같은 복잡한 질문에 대해 사용자가 이해하기 쉬운 **체계적인 설명을 제공**<br/>- **자연스러운 대화 기반 액션 구현:** GenUI 기술을 통해 사용자의 자연스러운 대화와 실제 액션(예: 게시글 수정) 사이의 간격을 줄여, 에이전트가 진정한 **'에이전트' 역할**을 수행|

* **벡터 검색 (Vector Search):** RAG를 구현하기 위해 Notion, FAQ, 콘텐츠 등의 내부 문서를 수집, 전처리하고 임베딩하여 **Pinecone DB**와 같은 벡터 데이터베이스에 저장하고 벡터 검색을 활용
* **GenUI (Generative User Interface):** 사용자의 의도에 따라 필요한 UI 요소를 즉시 생성하여 보여줄 수 있는 '생성형 UI' 개념을 KAMP에 적용. 기존의 고정된 UI에서 벗어나 사용자 대화 흐름에 맞춘 동적인 화면 제공
