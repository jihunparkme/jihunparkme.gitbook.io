...
알고리즘을 추가할 때마다 메인 클래스의 크기가 두 배로 늘어나고, 간단한 버그를 수정하거나 기능을 변경하면 전체 클래스에 영향이 미쳐 이미 작동하는 코드에서 오류가 발생할 가능성이 높아졌습니다.

그리고 새로운 기능을 구현하려면 거대한 클래스를 변경해야 하는데 이렇게 변경한 내용들이 다른 코드와의 병합 충돌로 많이 시간이 낭비되고 있습니다.

이 문제를 효율적으로 해결할 수 있는 방법은 없을까? 😭
...
- 객체 내에서 한 알고리즘의 다양한 변형들을 사용하고 싶을 경우.
  - 그리고 런타임 중 다른 알고리즘으로 전환하고 싶을 경우
- 일부 행동을 실행하는 방식에서만 차이가 있는 유사한 클래스가 많은 경우
- 콘텍스트에서 알고리즘 구현 세부 사항을 분리하고 싶을 경우
...
- 알고리즘을 사용하는 코드와 세부 구현의 분리
- 상속을 합성으로 대체
- 개방/폐쇄 원칙(OCP). 콘텍스트를 변경하지 않고도 새로운 전략 도입 가능

단점.
- 사용되는 알고리즘이 적고 변경될 일이 없다면, 패턴과 함께 사용되는 새로운 클래스와 인터페이스로 복잡성 증가
- 클라이언트는 적절한 전략을 선택할 수 있도록 전략 간의 차이점들을 알아야 함
- 패턴 사용을 위해 클래스와 인터페이스를 추가하지 않고 함수형으로 처리 가능
...
작업을 처리하는 클래스에 따라 행동을 선택하는 조건문이 많이 있다. 처리 클래스에 공통 인터페이스나 공통 클래스를 적용했더라면 클라이언트 코드에서 조건문을 제거하고 처리 객체에 메서드를 호출할 때 다형성을 사용할 수 있었을 것이다. 

이 상황을 효율적으로 해결할 수 있는 방법은 없을까? 😭
...
- 클라이언트가 알고리즘의 특정 단계들만 확장할 수 있도록 하고 싶고, 전체 알고리즘이나 알고리즘 구조는 확장하지 못하도록 하고 싶을 경우
  - 부모 클래스에서 정의된 구조를 그대로 유지하면서 자식 클래스로 쉽게 확장 가능
- 유사한 알고리즘을 포함하는 여러 클래스가 있는 경우 (결과적으로 알고리즘이 변경되면 모든 클래스를 수정해야 하는 경우)
...
장점.
- 클라이언트가 대규모 알고리즘의 특정 부분만 오버라이드하도록 하여 알고리즘의 다른 부분에 발생하는 변경에 영향을 덜 받도록 할 수 있음
- 중복 코드를 부모 클래스로 이전 가능

단점.
- 일부 클라이언트는 알고리즘의 제공된 골격에 제한
- 자식 클래스를 통해 디폴트 단계 구현을 억제하여 리스코프 치환 원칙 위반 가능
- 템플릿 메서드에 단계가 많을수록 유지가 더 어려운 경향 존재
...
그래프 데이터를 XML 형식으로 내보내는 작업을 구현해야 한다. 노드 클래스에 해당 기능을 추가하려고 했지만, 노드 클래스에 XML 내보내기 기능을 넣을 수 없는 상황이다. 또한, 데이터를 XML 형식이 아닌 다른 형식으로 내보내는 기능이 추가될 가능성이 있다고 한다. 그러면 새로운 기능이 추가될 떄마다 클래스를 변경해야 하는데..
...
- 트리와 같은 복잡한 객체 구조​의 모든 요소에 대한 작업이 필요할 경우
  - 같은 작업의 여러 변형을 구현하도록 하면서 다양한 클래스를 가진 여러 객체의 집합에 작업을 실행할 수 있도록 지원
- 보조 기능에 대한 비즈니스 로직을 비지터 클래스의 집합으로 분리
- 특정 기능이 클래스 계층구조의 일부 클래스에서만 의미가 있고 다른 클래스에서는 의미가 없을 경우
...
장점.
- 개방/폐쇄 원칙(OCP). 다른 클래스를 변경하지 않으면서 해당 클래스의 객체와 작동할 수 있는 새로운 행동 추가 가능
- 단일 책임 원칙(SRP). 동일한 기능의 여러 버전을 같은 클래스로 이동 가능
- 비지터 객체는 다양한 객체들과 작업하면서 유용한 정보를 축적 가능.
  - 객체 트리와 같은 복잡한 객체 구조를 순회하여 각 객체에 비지터 패턴을 적용하려는 경우 유용

단점.
- 클래스가 계층구조에 추가/제거될 때마다 모든 비지터에 대한 업데이트 필요
- 비지터들은 함께 작업해야 하는 요소의 비공개 필드/메서드에 접근하기 위한 권한이 부족할 수 있음