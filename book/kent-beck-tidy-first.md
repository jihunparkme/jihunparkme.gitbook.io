...
- 요소로 구성한 계층 구조에서 요소들은 `서로 관계를 지닌 존재`들입니다.
...
- 하나의 설계 작업은 작은 하위 요소로 만든 `거대한 수프`를 만드는 일과 같은 것이다.
- 설계할 때, 시스템에는 필요하지만 기계를 위한 명령어가 아닌, 일종의 중간 요소를 만들면, 그 중간 요소들이 서로에게 도움이 되기 시작합니다.
  - 함수 A는 함수 B가 계산의 복잡한 부분을 덜어 가면 더 간단해진다.

**요소들을 유익하게 관계 맺는 일**

- `설계`: 설계를 구성하는 **요소**들과 그들의 **관계**, 그리고 그 관계에서 파생되는 **이점**
- `설계자`: 설계자는 요소들을 유익하게 **관계 맺는 일**을 함
  - 요소를 만들고 삭제하기
  - 관계를 만들고 삭제하기
  - 관계의 이점 높이기
...
> 구조 변경과 동작 변경은 모두 가치를 마늗ㄹ어내지만, 근본적으로 다르다
>
> 가역성: 되될릴 수 있는 능력